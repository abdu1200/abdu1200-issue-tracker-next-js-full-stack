...
import { z } from 'zod';
import { IssueEditSchema } from "../../validationschemas";
...
type IssueFormData = z.infer<typeof IssueEditSchema>  //the right one returns a type and IssueFormData infer its type based on the schema



const EditIssuePage = ({issue}: {issue? : Issue}) => {     //the question mark is b/c the issue is optional herez( meaning: optional for newissuepage but must for editissuepage)
    ...
    //to register the fields with react-hook-form
     const {register, control, handleSubmit, formState:{errors}} = useForm<IssueFormData>({    // useForm hook func takes a configuration object
      resolver: zodResolver(IssueEditSchema)   //integration of zod with the react-hook-form
     })   //useForm returns an object  and register is a func that returns an object

     const router =  useRouter() //router is an object   //useRouter() is a router hook  
    ...
    //we use axios to send an update, create, delete request to an api(or api endpoint )
  ...
  return (
    <div className='max-w-xl'>
      ...
      <form 
          ...
          <select defaultValue={issue?.status} {...register("status")}  className="border border-gray-300 rounded-md p-1 w-full">
